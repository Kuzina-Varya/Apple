...
## Минимальный сценарий для демонстрации

```text
1. Приложение запускается на телефоне с ОС Аврора.
2. Делает GET /models.
3. Показывает 3 доступные модели.
4. Пользователь выбирает модель.
5. Приложение скачивает .tflite через /download.
6. Приложение сохраняет файл локально.
7. Приложение показывает, что модель выбрана и готова к работе.
```
