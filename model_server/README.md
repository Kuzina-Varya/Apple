...
## Запуск демонстрации одной командой

Для защиты используется файл:
...
2. Показать, что контейнер `model-server` работает.
3. Открыть Swagger:

```text
https://ninja-primp-late.ngrok-free.dev/docs
```

4. Показать `/models` — список доступных моделей.
5. Показать `/models/{model_id}` — полную информацию, включая размер и SHA256.
6. Показать `/models/{model_id}/download` — скачивание `.tflite`.
7. На телефоне с ОС Аврора показать получение списка моделей и скачивание выбранной модели.

## Формулировка для защиты

Модели хранятся на серверной стороне в директории `models_store`. Серверная часть реализована на FastAPI и развёрнута в Docker-контейнере. Мобильное приложение получает список моделей через endpoint `/models`, пользователь выбирает нужную модель, после чего приложение скачивает выбранный `.tflite` файл через `/models/{model_id}/download` и сохраняет его локально на устройстве. Для демонстрации на реальном телефоне с ОС Аврора локальный Docker-сервер опубликован через ngrok по постоянной HTTPS-ссылке. В промышленной версии этот же контейнер может быть перенесён на VPS или облачный сервер без изменения API.
